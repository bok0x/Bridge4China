...
export default function InterviewLeadsTab() {
  const [leads, setLeads] = useState<InterviewLead[]>([]);
  ...
  async function fetchLeads() {
    try {
      ...
      setError("Failed to load interview leads.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function updateStatus(id: string, status: string) {
    setUpdating(id);
    try {
      await fetch(`/api/interview-leads/${id}`, {
        method: "PATCH",
        headers: {
  ...
  function exportCSV() {
    const headers = ["Name", "Email", "WhatsApp", "Service Type", "Status", "Date"];
    const rows = filtered.map((l) => [
      l.fullName,
      l.email,
      l.whatsapp,
      SERVICE_TYPE_LABELS[l.serviceType] ?? l.serviceType,
      l.status,
    ...
    a.href = url;
    a.download = "interview-leads.csv";
    a.click();
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--glass-border)" }}>
                {["Name", "Email", "WhatsApp", "Service Type", "Status", "Date"].map((h) => (
                  <th
                    key={h}
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <span
                        ...
                        style={{
                          background:
                            lead.serviceType === "interview_prep"
                              ? "rgba(139,92,246,0.12)"
                              : "rgba(56,172,207,0.12)",
                          color:
                            lead.serviceType === "interview_prep" ? "#8b5cf6" : "#38ACCF",
                        }}
                      >
